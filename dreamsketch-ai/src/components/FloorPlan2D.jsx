...
function buildRenderPlan(rooms, floorWidth, floorHeight) {
  const walls = [
    { x1: 0, y1: 0, x2: floorWidth, y2: 0, thickness: 8, stroke: "#f8fafc" },
    { x1: floorWidth, y1: 0, x2: floorWidth, y2: floorHeight, thickness: 8, stroke: "#f8fafc" },
    { x1: floorWidth, y1: floorHeight, x2: 0, y2: floorHeight, thickness: 8, stroke: "#f8fafc" },
    { x1: 0, y1: floorHeight, x2: 0, y2: 0, thickness: 8, stroke: "#f8fafc" }
  ];
  ...
  edgeMap.forEach((entry) => {
    walls.push({
      ...
      x2: entry.x2,
      y2: entry.y2,
      thickness: entry.shared ? 4 : 2,
      stroke: entry.shared ? "#cbd5e1" : "#94a3b8"
    });
  });
  ...
  rooms.forEach((room) => {
    const edges = [
      ...
      },
      {
        ...
        y1: room.y,
        y2: room.y + room.length,
        exterior: Math.abs(room.x + room.width - (floorWidth - ROOM_PADDING)) < 2
      },
      {
        orientation: "horizontal",
        y: room.y,
      ...
      },
      {
        ...
        x1: room.x,
        x2: room.x + room.width,
        exterior: Math.abs(room.y + room.length - (floorHeight - ROOM_PADDING)) < 2
      }
    ];
...
export default function FloorPlan2D({ plan, onRoomDragEnd, onEntranceDragEnd }) {
  ...
  const renderPlan = useMemo(() => {
    ...
    return buildRenderPlan(rooms, floor.width, floor.length);
  }, [rooms, floor]);
    ...
    >
      ...
      >
        ...
        <g transform="translate(40, 40)">
          <rect x="0" y="0" width={floor.width} height={floor.length} fill="none" stroke="#64748b" strokeWidth="4" rx="8" />
